...
class unit_san(nn.Module):
    def __init__(self, in_channels, out_channels, A, groups, num_point, num_subset=3, **kwargs):
        ...
        self.attention0s = nn.Parameter(torch.ones(1, num_subset, self.inter_channels, num_point, num_point) / num_point,
                                                requires_grad=True)
        ...
        self.Linear_weight = nn.Parameter(torch.zeros(
            in_channels, self.inter_channels * num_subset, requires_grad=True, device='cuda'), requires_grad=True)
        nn.init.normal_(self.Linear_weight, 0, math.sqrt(
            0.5 / (self.inter_channels * num_subset)))

        self.Linear_bias = nn.Parameter(torch.zeros(
            1, self.inter_channels * num_subset, 1, 1, requires_grad=True, device='cuda'), requires_grad=True)
    ...
    def forward(self, x0):
        ...
        #x = self.norm1(x)
        q, k = torch.chunk(self.in_nets(x).view(n, 2 * self.num_subset, self.inter_channels, t, v), 2, dim=1)  # nctv -> n num_subset c'tv
        attention = self.tan(torch.einsum('nkctu,nkctv->nkuv', [q, k]) / (self.inter_channels * t)) * self.alphas 
...
class unit_tan(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=9, stride=1, num_point=25, block_size=41, window_size=120):
        super().__init__()
        pad = int((kernel_size - 1) / 2)
        #self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=(kernel_size, 1), padding=(pad, 0), stride=(stride, 1))
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=(1, 1), padding=(0, 0), stride=(1, 1))
        ...
        self.dropS = DropBlock_Ske(num_point=num_point)
        self.dropT = DropBlockT_1d(block_size=block_size)
        self.attention0s = nn.Parameter(torch.ones(1, out_channels, 1, kernel_size) / kernel_size,
                                                requires_grad=True)
    ...
    def forward(self, x, keep_prob, A):
        ...
        attention = F.softmax(torch.einsum('nctv,nctuv->nctu', x, upfold)/v , -1)  + self.attention0s
...
class global_tan(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=9, stride=1, num_point=25, block_size=41, window_size=120):
        ...
        #self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=(kernel_size, 1), padding=(pad, 0), stride=(stride, 1))
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=(1, 1), padding=(0, 0), stride=(1, 1))
        self.conv2 = nn.Conv2d(out_channels, out_channels, kernel_size=(1, 1), padding=(0, 0), stride=(1, 1))
        self.norm = nn.LayerNorm(out_channels)
        #self.norm = nn.BatchNorm2d(out_channels)
        self.pool = nn.AvgPool2d((stride, 1), (stride, 1))
        conv_init(self.conv)
        ...
        self.dropS = DropBlock_Ske(num_point=num_point)
        self.dropT = DropBlockT_1d(block_size=block_size)
        self.attention0s = nn.Parameter(torch.zeros(1, out_channels, window_size, window_size) + torch.eye(window_size), requires_grad=True)
    ...
    def forward(self, x, keep_prob, A):
        ...
        attention = F.softmax(torch.einsum('nctv,ncyv->ncty', x, x_global)/v , -1)  + self.attention0s
        #print(attention[0,0,:5,:5])
...
class Edge_conv(nn.Module):
    def __init__(self, in_channels, num_joint=27, edge_num=6):
        ...
        self.joint_edge_connections = nn.Parameter(torch.randn(num_joint, edge_num), requires_grad=True)
        self.edge_joint_connections = nn.Parameter(torch.randn(edge_num, num_joint), requires_grad=True)
        trunc_normal_(self.joint_edge_connections, std=.02)
        trunc_normal_(self.edge_joint_connections, std=.02)
...
class Edge_feature_conv(nn.Module):
    def __init__(self, in_channels, num_joint=27, edge_num=6):
        ...
        trunc_normal_(self.edge_features, std=.02)
        self.norm1 = nn.LayerNorm(in_channels)
        self.transform = nn.Conv2d(in_channels, in_channels, kernel_size=1)
    ...
    def forward(self, x):
        ...
        x = rearrange(x, '(b t) v c -> b c t v', b=n)
        x = self.transform(x)
        attention = F.tanh(torch.einsum('nctv, ce->ntev', x, self.edge_features)) /(c) * self.alphas 
...
class Block(nn.Module):
    def __init__(self, in_channels, out_channels, A, groups, num_point, block_size, stride=1, residual=True, use_grpe=True, is_first=False, window_size=120, **kwargs):
...
class unit_tcn_dilated(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=9, stride=1, dilation=1, num_point=25, block_size=41):
        super().__init__()
        pad = (kernel_size + (kernel_size-1) * (dilation-1) - 1) // 2
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=(kernel_size, 1), padding=(pad, 0),
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, groups=8, block_size=41, graph=None, graph_args=dict(), in_channels=3,  inner_dim=64, drop_layers=3, depth=4, s_num_heads=1, window_size=120):
