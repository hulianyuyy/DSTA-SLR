...
def get_parser():
    ...
    parser.add_argument(
        '--dataset', default='WLASL2000', choices=['WLASL100', 'WLASL300', 'WLASL1000', 'WLASL2000', 'AUTSL', 'SLR500'], help='dataset name')
    parser.add_argument(
        '--save-score',
        type=str2bool,
        default=False,
...
class Processor():
    ...
    def __init__(self, arg):
        ...
        if self.arg.wandb:
            wandb.init(name=self.arg.wandb_name, entity=self.arg.wandb_entity, project=self.arg.wandb_project, config=self.arg)
    ...
    def load_data(self):
        ...
        if self.arg.phase == 'train':
            self.data_loader['train'] = torch.utils.data.DataLoader(
                dataset=Feeder(**self.arg.train_feeder_args, num_class=self.arg.model_args['num_class']),
        ...
        self.data_loader['test'] = torch.utils.data.DataLoader(
            dataset=Feeder(**self.arg.test_feeder_args, num_class=self.arg.model_args['num_class']),
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        with torch.no_grad():
            ...
            for ln in loader_name:
                ...
                accuracy_per_class = self.data_loader[ln].dataset.per_class_acc_top_k(score, 1)
                accuracy_5_per_class = self.data_loader[ln].dataset.per_class_acc_top_k(score, 5)
                if accuracy > self.best_acc:
                    ...
                    with open('./work_dir/' + self.arg.Experiment_name + '/eval_results/best_acc' + '.pkl'.format(
                ...
                self.print_log('Eval Accuracy: {}, model: {}'.format(accuracy, self.arg.model_saved_name))
                self.print_log(f"torch metrics acc: {(100 * total_acc):>0.1f}%\n")
                self.print_log("recall of every test dataset class: ", total_recall)
                self.print_log("precision of every test dataset class: ", total_precision)
                self.print_log(f"f1 score: {2*total_recall*total_precision/(total_recall+total_precision)}")
                ...
                for k in self.arg.show_topk:
                    ...
                    self.print_log('\tTop{} per-class: {:.2f}%'.format(
                        k, 100 * self.data_loader[ln].dataset.per_class_acc_top_k(score, k)))

                with open('./work_dir/' + self.arg.Experiment_name + '/eval_results/epoch_' + str(epoch) + '_' + str(accuracy) + '.pkl'.format(
                        epoch, accuracy), 'wb') as f:
                    pickle.dump(score_dict, f)
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            self.print_log('best accuracy: {}, best top-5 accuracy: {}, best accuracy per-class: {}, best top-5 accuracy per-class: {}, model_name: {}'.format(self.best_acc, self.best_acc_5, self.best_accuracy_per_class, self.best_accuracy_5_per_class, self.arg.model_saved_name))
